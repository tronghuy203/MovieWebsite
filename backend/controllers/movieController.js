...
const movieController = {
  addMovie: async (req, res) => {
    try {
      ...
      if (files.poster) {
        posterUrl = await uploadToCloudinary(files.poster[0], "movies/posters");
      }
      if (files.poster2) {
        posterUrl2 = await uploadToCloudinary(files.poster2[0], "movies/posters");
      }
      if (files.trailer) {
        trailerUrl = await uploadToCloudinary(files.trailer[0], "movies/trailers");
      }
      if (files.video) {
        videoUrl = await uploadToCloudinary(files.video[0], "movies/videos");
      }
  ...
  updateMovie: async (req, res) => {
    try {
      ...
      if (files.poster) {
        ...
        updateData.posterUrl = await uploadToCloudinary(files.poster[0], "movies/posters");
      }
      if (files.poster2) {
        await deleteFromCloudinary(movie.posterUrl2);
        updateData.posterUrl2 = await uploadToCloudinary(files.poster2[0], "movies/posters");
      }
      if (files.trailer) {
        await deleteFromCloudinary(movie.trailerUrl);
        updateData.trailerUrl = await uploadToCloudinary(files.trailer[0], "movies/trailers");
      }
      if (files.video) {
        await deleteFromCloudinary(movie.videoUrl);
        updateData.videoUrl = await uploadToCloudinary(files.video[0], "movies/videos");
      }
